...
A comic maker for kids. **The kid draws everything** — characters, backdrops, props, effects — saves them to a Toy Box, and builds multi-page comics. The Story Helper only ever brings **words**: story plans, dialogue, and dares to draw more. It never generates art.
...
1. **Push to GitHub**
   ```bash
   git init && git add -A && git commit -m "Scribble Studio"
   git remote add origin https://github.com/YOUR_USERNAME/scribble-studio.git
