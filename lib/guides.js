...
export function drawGuide(gctx, W, H, subType) {
  ...
  const frames = {
    ...
    /* ---- Backdrop Bits kit frames ---- */
    cloud() {
      circle(cx - W * 0.14, cy, W * 0.12);
      circle(cx + W * 0.02, cy - H * 0.07, W * 0.15);
      circle(cx + W * 0.17, cy, W * 0.11);
      line(cx - W * 0.26, cy + H * 0.1, cx + W * 0.28, cy + H * 0.1);
      label('a puffy cloud', W * 0.06, H * 0.12);
    },
    sun() {
      circle(cx, cy, W * 0.16);
      for (let i = 0; i < 8; i++) {
        const a = (Math.PI * 2 * i) / 8;
        line(cx + Math.cos(a) * W * 0.2, cy + Math.sin(a) * W * 0.2,
             cx + Math.cos(a) * W * 0.3, cy + Math.sin(a) * W * 0.3);
      }
      label('a sunny sun', W * 0.06, H * 0.12);
    },
    tree() {
      line(cx - W * 0.03, H * 0.85, cx - W * 0.03, cy + H * 0.02);
      line(cx + W * 0.03, H * 0.85, cx + W * 0.03, cy + H * 0.02);
      circle(cx, cy - H * 0.12, W * 0.2);
      label('a tree — trunk + leafy top', W * 0.05, H * 0.12);
    },
    star() {
      gctx.beginPath();
      const sp = 5, ro = W * 0.26, ri = W * 0.11;
      for (let i = 0; i < sp * 2; i++) {
        const a = (Math.PI / sp) * i - Math.PI / 2;
        const r = i % 2 ? ri : ro;
      ...
      gctx.closePath(); gctx.stroke();
      label('a star — wonky is perfect!', W * 0.05, H * 0.12);
    },
    rock() {
      gctx.beginPath();
      gctx.ellipse(cx, cy + H * 0.12, W * 0.26, H * 0.18, 0, Math.PI, Math.PI * 2);
      gctx.stroke();
      line(cx - W * 0.26, cy + H * 0.12, cx + W * 0.26, cy + H * 0.12);
      label('a lumpy rock', W * 0.06, H * 0.12);
    },
  };
  frames.flower = frames.nature;
