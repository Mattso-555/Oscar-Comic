...
export function cleanupBackground(ctx, w, h) {
  ...
  // --- Pass 1: white-balance every photo pixel; record luminance ---
  const N = w * h;
  const lum = new Float32Array(N);
  const letter = new Uint8Array(N); // letterbox (outside the photo itself)
  for (let y = 0; y < h; y++) {
    const fy = Math.max(0, Math.min(G - 1, (y / h) * G - 0.5));
    for (let x = 0; x < w; x++) {
      const s = y * w + x, i = s * 4;
      if (p[i + 3] === 0) { letter[s] = 1; lum[s] = 255; continue; }
  ...
  // --- Pass 2: STICKER CUT — only paper connected to the outside goes clear.
  // Paper pockets enclosed by the drawing's outline stay solid (clean white),
  // so characters aren't see-through. Outline gaps up to ~2×gap px are sealed.
  const PAPER = 208, gap = 3;
  let barrier = new Uint8Array(N); // the drawing (ink), thickened to seal gaps
  for (let s = 0; s < N; s++) if (!letter[s] && lum[s] < PAPER) barrier[s] = 1;
  for (let pass = 0; pass < gap; pass++) {
    ...
    for (let y = 0; y < h; y++) {
      ...
      for (let x = 0; x < w; x++) {
        ...
        if ((x > 0 && barrier[s - 1]) || (x < w - 1 && barrier[s + 1]) ||
            (y > 0 && barrier[s - w]) || (y < h - 1 && barrier[s + w])) next[s] = 1;
      }
    }
    barrier = next;
  }
  ...
  const stack = [];
  const seed = (s) => { if (!outside[s] && !barrier[s]) { outside[s] = 1; stack.push(s); } };
  for (let s = 0; s < N; s++) if (letter[s]) { outside[s] = 1; stack.push(s); }
  for (let x = 0; x < w; x++) { seed(x); seed((h - 1) * w + x); }
  for (let y = 0; y < h; y++) { seed(y * w); seed(y * w + w - 1); }
  while (stack.length) {
    const s = stack.pop();
    const x = s % w, y = (s / w) | 0;
    if (x > 0 && !outside[s - 1] && !barrier[s - 1]) { outside[s - 1] = 1; stack.push(s - 1); }
    if (x < w - 1 && !outside[s + 1] && !barrier[s + 1]) { outside[s + 1] = 1; stack.push(s + 1); }
    if (y > 0 && !outside[s - w] && !barrier[s - w]) { outside[s - w] = 1; stack.push(s - w); }
    if (y < h - 1 && !outside[s + w] && !barrier[s + w]) { outside[s + w] = 1; stack.push(s + w); }
  }
  // grow the outside back across true paper to meet the real ink edge
  for (let pass = 0; pass < gap + 1; pass++) {
    ...
    for (let y = 0; y < h; y++) {
      ...
      for (let x = 0; x < w; x++) {
        const s = row + x;
        if (outside[s] || letter[s] || lum[s] < PAPER) continue;
        if ((x > 0 && outside[s - 1]) || (x < w - 1 && outside[s + 1]) ||
            (y > 0 && outside[s - w]) || (y < h - 1 && outside[s + w])) adds.push(s);
      }
    }
    if (!adds.length) break;
    for (const s of adds) outside[s] = 1;
  }

  // --- Pass 3: apply — outside fades away; inside stays solid, paper→white ---
  for (let s = 0; s < N; s++) {
    if (letter[s]) continue;
    const i = s * 4, L = lum[s];
    if (outside[s]) {
      if (L >= 232) p[i + 3] = 0;
      else if (L > 170) p[i + 3] = Math.round(((232 - L) / 62) ** 1.3 * 255);
      else p[i + 3] = 255;
    } else {
      p[i + 3] = 255;
      if (L >= 232) { p[i] = 255; p[i + 1] = 255; p[i + 2] = 255; } // sticker interior
    }
  }
...
/**
 * Does this sprite still need cutting out? Properly cut drawings always have
 * a transparent border ring (the crop adds padding), so an opaque edge is the
 * reliable tell of an uncut photo — much more robust than total coverage,
 * which the old pipeline's hotspot-holes could drag below any threshold.
 */
export async function looksUncut(dataUrl) {
  const img = await loadImage(dataUrl);
  const S = 96;
  const s = Math.min(S / img.width, S / img.height);
  ...
  const p = c.getImageData(0, 0, w, h).data;
  const ring = 3;
  let edgeOpaque = 0, edgeTotal = 0, opaque = 0;
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const a = p[(y * w + x) * 4 + 3];
      if (a > 200) opaque++;
  ...
  return edgeOpaque / edgeTotal > 0.25 || opaque / (w * h) > 0.82;
}
