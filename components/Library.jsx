...
import { cleanupBackground, exportSprite, opaqueCoverage, recutDataUrl } from '@/lib/sprites';
...
export default function Library({ items, removeItem, renameItem, updateItem, addItems, flash }) {
  ...
  const autoCutout = async () => {
    ...
    for (const it of items) {
      try { if ((await opaqueCoverage(it.dataUrl)) > 0.82) targets.push(it); } catch {}
    }
    if (!targets.length) {
      setCutting(null);
