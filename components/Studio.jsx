...
// Backdrop Bits kit: quick guided prompts. Each bit the kid draws auto-saves
// to the Toy Box, ready to scatter into backdrops forever. All their art.
const KIT = [
  { name: 'Cloud', emoji: '☁️', hint: 'Trace the faint cloud — big bumpy bumps!', guide: 'cloud' },
  { name: 'Sun', emoji: '☀️', hint: 'Trace the sun — rays shooting out!', guide: 'sun' },
  { name: 'Tree', emoji: '🌳', hint: 'A trunk with a big leafy top!', guide: 'tree' },
  { name: 'Star', emoji: '⭐', hint: 'A star — spiky or wonky is perfect!', guide: 'star' },
  { name: 'Rock', emoji: '🪨', hint: 'A lumpy rock or a little hill!', guide: 'rock' },
  { name: 'Flower', emoji: '🌸', hint: 'A flower with happy petals!', guide: 'flower' },
];

export default function Studio({ beginner, addItem, flash, editItem, endEdit, updateItem }) {
  ...
  const [kit, setKit] = useState(null); // null | index into KIT
  ...
  // guide layer redraw — kit bits always show their trace outline
  useEffect(() => {
    const g = guideRef.current.getContext('2d');
    if (kit !== null) drawGuide(g, SIZE, SIZE, KIT[kit].guide);
    else if (beginner && guideOn) drawGuide(g, SIZE, SIZE, subType);
    else g.clearRect(0, 0, SIZE, SIZE);
  }, [beginner, guideOn, subType, kit]);

  // reset coach on subtype change
  useEffect(() => { setCoachStep(0); }, [subType, kind]);

  // load a Toy Box item onto the canvas for editing
  useEffect(() => {
    if (!editItem || !ctxRef.current) return;
    const ctx = ctxRef.current;
    const img = new Image();
    img.onload = () => {
      ctx.clearRect(0, 0, SIZE, SIZE);
      undoStack.current = [];
      const s = Math.min((SIZE * 0.92) / img.width, (SIZE * 0.92) / img.height);
      const w = img.width * s, h = img.height * s;
      ctx.drawImage(img, (SIZE - w) / 2, (SIZE - h) / 2, w, h);
    };
    img.src = editItem.dataUrl;
  }, [editItem]);

  const saveEdit = () => {
    // pop:false — the original already had its colour boost at first save
    const data = exportSprite(ctxRef.current, SIZE, SIZE, { pop: false });
    if (!data) { flash('Nothing left to save — Undo to bring it back!'); return; }
    updateItem(editItem.id, { dataUrl: data });
    ctxRef.current.clearRect(0, 0, SIZE, SIZE);
    undoStack.current = [];
    flash(`${editItem.name} updated! ✨`);
    endEdit();
  };
  const cancelEdit = () => {
    ctxRef.current.clearRect(0, 0, SIZE, SIZE);
  ...
  const start = (e) => {
    ...
    if (tool === 'fill') {
      snapshot();
      const filled = floodFill(ctxRef.current, SIZE, SIZE, p.x, p.y, color);
      // Guard: flooding nearly the whole page is almost never what a kid
      // wants unless they're painting a backdrop. Revert and coach instead.
      const wholePage = filled / (SIZE * SIZE) > 0.88;
      if (wholePage && (kit !== null || kind !== 'backdrop')) {
        ctxRef.current.putImageData(undoStack.current.pop(), 0, 0);
        flash('Draw your shape first, then tap inside it to colour! 🪣');
      }
      return;
    }
    ...
    last.current = p;
    mid.current = p;
    // draw a dot immediately so single taps mark (or erase) too
    const ctx = ctxRef.current;
    ctx.globalCompositeOperation = tool === 'eraser' ? 'destination-out' : 'source-over';
    ctx.fillStyle = tool === 'eraser' ? '#000' : color;
    ctx.beginPath();
    ctx.arc(p.x, p.y, (tool === 'eraser' ? size * 2.4 : size) / 2, 0, Math.PI * 2);
    ctx.fill();
  ...
  const move = (e) => {
    ...
    // Eraser note: destination-out removes pixels where the SOURCE is opaque,
    // so the eraser stroke must be a solid colour — transparent strokes erase nothing.
  ...
  const saveBit = () => {
    const data = exportSprite(ctxRef.current, SIZE, SIZE, { pop: true });
    if (!data) { flash('Draw your bit first! ✏️'); return; }
    const bit = KIT[kit];
    addItem({ id: Date.now(), name: bit.name, tag: 'prop', subType: 'nature', dataUrl: data });
    ctxRef.current.clearRect(0, 0, SIZE, SIZE);
    undoStack.current = [];
    if (kit + 1 < KIT.length) {
      setKit(kit + 1);
      flash(`${bit.emoji} ${bit.name} saved! Next…`);
    } else {
      setKit(null);
      flash('🎉 Kit complete! Scatter your bits in the Comic!');
    }
  };
  const skipBit = () => {
    if (kit + 1 < KIT.length) setKit(kit + 1);
    else { setKit(null); flash('🎉 Kit finished!'); }
  };
  ...
  return (
    <div className="studio">
      ...
      <div className="stack tools">
        {editItem ? (
          <div className="panelcard editcard">
            <h3>✏️ Editing: {editItem.name}</h3>
            <div className="hint" style={{ marginTop: 0 }}>
              Erase stray marks, clean the edges with the 🧽 Eraser, recolour with the 🪣 Fill — then save your changes.
            </div>
            <div className="row" style={{ marginTop: 8 }}>
              <button className="btn sm green" onClick={saveEdit}>💾 Save changes</button>
              <button className="btn sm" onClick={cancelEdit}>✖ Stop editing</button>
            </div>
          </div>
        ) : kit === null ? (
          <div className="panelcard kitcard">
            <h3>🧩 Backdrop Bits</h3>
            <div className="hint" style={{ marginTop: 0 }}>
              Draw 6 quick bits — clouds, trees, stars — then scatter them into backdrops forever. About 30 seconds each!
            </div>
            <button className="btn green" style={{ width: '100%', justifyContent: 'center', marginTop: 8 }} onClick={startKit}>
              ▶️ Start the kit
            </button>
          </div>
        ) : (
          <div className="panelcard kitcard active">
            <h3>🧩 Bit {kit + 1} of {KIT.length}: {KIT[kit].emoji} {KIT[kit].name}</h3>
            <div className="hint" style={{ marginTop: 0 }}>{KIT[kit].hint}</div>
            <div className="row" style={{ marginTop: 8 }}>
              <button className="btn sm green" onClick={saveBit}>✅ Save bit — next!</button>
              <button className="btn sm" onClick={skipBit}>⏭️ Skip</button>
              <button className="btn sm red" onClick={() => setKit(null)}>✖ Stop</button>
            </div>
            <div className="hint">Trace the faint shape on the paper, then make it your own — the guide never gets saved!</div>
          </div>
        )}
      ...
      <style jsx>{`
        ...
        :global(.kitcard) { background: linear-gradient(165deg, #fff7e0, #fff); }
        :global(.kitcard.active) { outline: 3px solid var(--yellow); outline-offset: 2px; }
        :global(.editcard) { background: linear-gradient(165deg, #eaf3ff, #fff); outline: 3px solid var(--blue); outline-offset: 2px; }
