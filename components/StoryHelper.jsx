...
export default function StoryHelper({ items, applyPlan, addBubble, goDraw, flash }) {
  ...
  const run = async (kind) => {
    ...
    try {
      let text;
      if (kind === 'plan') {
        text = await askClaude(`My cast and world: ${cast}. Invent a genuinely surprising short comic story starring MY drawings, each with a funny personality. Give it a hook, a twist nobody sees coming, and a satisfying silly ending. Break it into 4-9 panels, one vivid sentence each. CRITICAL: whenever a drawing appears in a panel, use its EXACT name from my cast list (the app auto-places drawings whose names appear in each sentence) — and weave in my props, weapons, vehicles and places by exact name too, not just characters. ${dare} Return ONLY JSON: {"title":"...","panels":["...","..."]}.`);
  ...
  return (
    <div className="panelcard helper">
      ...
      <div className="out">
        ...
        {!busy && out && out.kind === 'plan' && (
          <>
            ...
            {out.panels.map((t, i) => (
              <div key={i} className="beat"><b>Panel {i + 1}:</b> {t}</div>
            ))}
            <button className="btn sm green" style={{ marginTop: 8 }} onClick={() => applyPlan(out)}>
              📖 Build this story with my drawings
            </button>
            <div className="note">Your drawings get placed into the panels — arrange them and draw what's missing!</div>
          </>
        )}
      ...
      <style jsx>{`
        ...
        .beat { padding: 5px 4px; font-weight: 700; }
