...
export default function StoryHelper({ items, applyPlan, addBubble, goDraw, flash, stories = [], addStory, removeStory }) {
  ...
  const run = async (kind) => {
    ...
    try {
      ...
      if (kind === 'plan') {
        ...
        if (addStory) addStory({ title: plan.title, panels: plan.panels.slice(0, 9) });
      } else {
        const prompts = {
      ...
      if (kind === 'plan') {
        const plan = offlinePlan(items);
        setOut({ kind, ...plan });
        if (addStory) addStory({ title: plan.title, panels: plan.panels });
      }
    ...
    <div className="panelcard helper">
      ...
      {stories.length > 0 && (
        <div className="storybook">
          <div className="sb-title">📚 Story Book — tap to reopen</div>
          {stories.map((s) => (
            <div key={s.id} className="sb-row">
              <button className="sb-open" onClick={() => setOut({ kind: 'plan', title: s.title, panels: s.panels })}>
                “{s.title}” <em>{s.panels.length} panels</em>
              </button>
              {removeStory && (
                <button className="sb-x" title="Forget this story" onClick={() => removeStory(s.id)}>×</button>
              )}
            </div>
          ))}
        </div>
      ...
      <style jsx>{`
        ...
        .storybook {
          margin-top: 10px; background: #fff; border: 2px solid var(--ink);
          border-radius: 10px; padding: 8px 10px;
        }
        .sb-title { font-weight: 800; font-size: 12px; color: var(--purple); margin-bottom: 4px; }
        .sb-row { display: flex; align-items: center; gap: 6px; }
        .sb-open {
          flex: 1; text-align: left; border: none; background: none; cursor: pointer;
          font: inherit; font-weight: 700; font-size: 12.5px; padding: 5px 4px; border-radius: 6px;
        }
        .sb-open:hover { background: #f1ecff; }
        .sb-open em { font-style: normal; font-size: 10.5px; color: #a9a695; margin-left: 4px; }
        .sb-x {
          width: 20px; height: 20px; border-radius: 50%; border: 2px solid var(--ink);
          background: #fff; font-weight: 800; line-height: 1; cursor: pointer; font-size: 11px;
        }
        .sb-x:hover { background: var(--red); color: #fff; }
        .spin {
