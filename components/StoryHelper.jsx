...
export default function StoryHelper({ items, applyPlan, addBubble, goDraw, flash }) {
  ...
  const run = async (kind) => {
    ...
    try {
      let text;
      if (kind === 'plan') {
        text = await askClaude(`My cast and world: ${cast}. Invent a genuinely surprising short comic story starring MY drawings by name, each with a funny personality. Give it a hook, a twist nobody sees coming, and a satisfying silly ending. Break it into 4-9 panels, one vivid sentence each. ${dare} Return ONLY JSON: {"title":"...","panels":["...","..."]}.`);
        ...
        {!busy && out && out.kind === 'plan' && (
          <>
            ...
            <button className="btn sm green" style={{ marginTop: 8 }} onClick={() => applyPlan(out)}>
              📖 Put this story in my comic
            </button>
            <div className="note">Captions go straight into your panels — you draw every picture!</div>
          </>
        )}
